...
if sys.version_info[0] != 3:
    raise Exception(
        "Blockly build only compatible with Python 3.x.\n"
        "You are using: " + sys.version
    )

import errno, glob, fnmatch, http.client, json, os, re, subprocess, threading, urllib, importlib
...
class Gen_langfiles(threading.Thread):
    ...
    def generate_blockly(self):
        ...
        try:
            ...
            json_files = [
                ...
                if not (
                    file.endswith(
                        ("keys.json", "synonyms.json", "qqq.json", "_ardublockly.json")
                    )
                )
            ]
    ...
    def generate_ardublockly(self):
        ...
        # The files msg/json/{en,qqq,synonyms}.json depend on msg/messages.js.
        if self._rebuild(
            [os.path.join("msg", "messages_ardublockly.js")],
            [
                os.path.join("msg", "json", f)
                for f in [
                    "en_ardublockly.json",
        ...
        try:
            ...
            cmd = [
                ...
                os.path.join("i18n", "create_messages.py"),
                "--source_lang_file",
                os.path.join("msg", "json", "en_ardublockly.json"),
                "--source_synonym_file",
                os.path.join("msg", "json", "synonyms_ardublockly.json"),
                "--output_dir",
                os.path.join("msg", "js"),
                "--ardublockly",
                "--quiet",
